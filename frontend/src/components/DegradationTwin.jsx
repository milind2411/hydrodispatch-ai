  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-2xl">
            ...
            {['PEM', 'Alkaline', 'SOEC'].map((t) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition ${
                  selectedTech === t ? 'bg-indigo-500 text-white shadow-md' : 'text-zinc-400 hover:text-white'
                }`}
              >
                {t} Stack
              </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800/80">
            <span className="text-[11px] text-zinc-400 uppercase font-semibold">Membrane Resistance</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl font-black text-white font-mono">0.24</span>
              <span className="text-xs text-zinc-400">Ω·cm²</span>
            </div>
          ...
          <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800/80">
            <span className="text-[11px] text-zinc-400 uppercase font-semibold">Remaining Useful Life (RUL)</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl font-black text-indigo-400 font-mono">43,200</span>
          ...
          <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800/80">
            <span className="text-[11px] text-zinc-400 uppercase font-semibold">Thermodynamic Eff (HHV)</span>
            <div className="flex items-baseline gap-1 mt-1">
          ...
          <div className="bg-zinc-900/60 p-3 rounded-xl border border-zinc-800/80">
            <span className="text-[11px] text-zinc-400 uppercase font-semibold">Health Score</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl font-black text-emerald-400 font-mono">96.8 / 100</span>
            </div>
            <div className="text-[10px] text-zinc-400 mt-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
          <div>
            <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wide">
              Cell Polarization Curve ($V_{'{cell}'}$ vs. Current Density $i$)
            </h3>
            <p className="text-xs text-zinc-400">
              Comparing Begin-of-Life (BOL Fresh) vs. End-of-Life (EOL Aged) membrane overpotential
            </p>
          </div>
          <span className="text-xs bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2.5 py-1 rounded-lg font-mono">
        ...
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={polarizationData} margin={{ top: 10, right: 15, left: -15, bottom: 0 }}>
              ...
              <XAxis dataKey="currentDensity" stroke="#71717a" tick={{ fontSize: 11 }} unit=" A/cm²" />
              <YAxis stroke="#71717a" tick={{ fontSize: 11 }} domain={[1.2, 2.4]} unit=" V" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-2xl">
          <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wide mb-1">
            Faraday Efficiency vs. Current Density (%)
          </h3>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={polarizationData} margin={{ top: 10, right: 10, left: -15, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="#27272a" opacity={0.6} />
                <XAxis dataKey="currentDensity" stroke="#71717a" tick={{ fontSize: 10 }} unit=" A/cm²" />
                <YAxis stroke="#71717a" tick={{ fontSize: 10 }} domain={[80, 100]} unit=" %" />
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-2xl flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3 text-xs text-zinc-300">
              <div className="p-3 bg-zinc-900/60 rounded-xl border border-zinc-800">
                ...
                <p className="text-zinc-400 mt-0.5">Rapid temperature gradients during sudden solar cloud drops induce mechanical shear in the Catalyst Coated Membrane (CCM). The MILP ramp constraint keeps ΔT &lt; 0.5°C/min.</p>
              </div>

              <div className="p-3 bg-zinc-900/60 rounded-xl border border-zinc-800">
                <strong className="text-emerald-300">2. Catalyst Dissolution Prevention:</strong>
                <p className="text-zinc-400 mt-0.5">High-frequency potential swings cause Platinum/Iridium dissolution at the anode. HydroDispatch AI enforces minimum up/down time states to avoid micro-cycling.</p>
              </div>

              <div className="p-3 bg-zinc-900/60 rounded-xl border border-zinc-800">
                <strong className="text-indigo-300">3. Gas Crossover Avoidance:</strong>
                <p className="text-zinc-400 mt-0.5">Operating below 10% minimum turndown increases H₂ → O₂ membrane permeation. The solver automatically draws minimum grid power or smoothly shuts down safely.</p>
              </div>
            </div>
          </div>
        </div>
